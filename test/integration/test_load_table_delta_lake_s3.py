...
class TestLoadTableDeltaLakeS3(S3TableTestBase):
    ...
    def env_setup(
        ...
        return m3d_config_file, scon_emr_file, tconx_file, tconx_cl_file, m3d_config_dict, scon_emr_dict
    ...
    def test_load_table_delta_lake(self, remove_json_patch, add_tags_patch, _0, _1):
        ...
        load_type = "DeltaLakeLoad"

        src_tconx_path = "test/resources/test_load_table_delta_lake_s3/tconx-bdp-emr_test-dev-bi_test101.json"
        src_tconx_cl_table = "test/resources/test_load_table_delta_lake_s3/tconx-bdp-emr_test-dev-bi_test101_cl.json"
        ...
        filename_json = "delta_lake_load-{environment}-{table}.json".format(
            environment=destination_environment,
            table=destination_active_table
        )

        # Checking configuration file for m3d-engine
        app_files = self.get_child_objects(s3_table_active.dir_apps_delta_lake_load)

        assert len(app_files) == 1

        assert app_files[0] == s3_table_active.dir_apps_delta_lake_load + filename_json
        ...
        assert load_table_parameters["records_to_condense"] == [None, "", "N", "R", "D", "X"]
        assert load_table_parameters["records_to_delete"] == ["R", "D", "X"]
        ...
        assert load_table_parameters["target_table"] == "{}.{}_{}".format(
            s3_table_active.db_lake, s3_table_active.source_system, s3_table_active.table)
        ...
        assert delta_load_step.args[-3] == "DeltaLakeLoad"
        ...
        assert sorted(add_tags_patch_call_args_list[0][0][0], key=lambda x: x["Key"]) == sorted([
            ...
            {"Key": "LoadType", "Value": "DeltaLakeLoad"},
